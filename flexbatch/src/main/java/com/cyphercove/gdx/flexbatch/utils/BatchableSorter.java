 ...
 * <p> 
 * Opaque Batchables are sorted by texture configuration to minimize flushes and drawn first. Blended Batchables
 * are sorted by distance from camera and drawn far to near.
 * 
 * @author cypherdare */
public class BatchableSorter<T extends Batchable & SortableBatchable> {
	...
	public BatchableSorter (Camera camera, int opaqueIntialTextureCapacity, int opaqueInitialCapacityPerTexture, int blendedInitialCapacity){
	...
	/** Sort, draw, and clear references to the queued Batchables. Must be called in between {@link FlexBatch#begin()}
	 * and {@link FlexBatch#end()}. */
