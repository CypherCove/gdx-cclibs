 ...
 * <p>
 * Pending state changes can be made at any time, but can only be applied by calling {@link #executeChanges()} between
 * calls to {@link #begin()} and {@link #end()}.
 * 
 * @author cypherdare */
public class RenderContextAccumulator {
	...
	public boolean hasPendingChanges () {
		...
		if (pending.blending) {
			if (pending.blendSrcFuncColor != current.blendSrcFuncColor || 
				pending.blendDstFuncColor != current.blendDstFuncColor
				|| pending.blendSrcFuncAlpha != current.blendSrcFuncAlpha
				|| pending.blendDstFuncAlpha != current.blendDstFuncAlpha) return true;
			if (pending.blendEquationColor != current.blendEquationColor ||
				pending.blendEquationAlpha != current.blendEquationAlpha) return true;
		}
	...
	/** Applies the pending state changes and texture bindings to GL. This must be called in between {@link #begin()} and {@link #end()}.*/
	public void executeChanges () {
		...
		if (pending.depthTesting) {
			...
			if (pending.depthRangeNear != current.depthRangeNear
				|| pending.depthRangeFar != current.depthRangeFar) {
		...
		if (pending.blending) {
			if (pending.blendSrcFuncColor != current.blendSrcFuncColor ||
				pending.blendDstFuncColor != current.blendDstFuncColor ||
				pending.blendSrcFuncAlpha != current.blendSrcFuncAlpha ||
				pending.blendDstFuncAlpha != current.blendDstFuncAlpha) {
				if (pending.blendSrcFuncColor == pending.blendSrcFuncAlpha &&
					...
					Gdx.gl.glBlendFuncSeparate(pending.blendSrcFuncColor, pending.blendDstFuncColor,
						pending.blendSrcFuncAlpha, pending.blendDstFuncAlpha);
				}
	...
	/** Returns actual OpenGL states to defaults. The blend function parameters, depth test function parameters, and culled face parameter are left
	 * unchanged. */
	public void end () {
		State temp = pending;
		pending = DEF;
	...
	/** Sets the depth test function and a depth range of 0 to 1. These parameters will only be applied if depth testing is enabled.
	...
	public boolean setDepthFunction (int depthFunc, float depthRangeNear, float depthRangeFar) {
		if (pending.depthFunc != depthFunc || pending.depthRangeNear != depthRangeNear
	...
	/** Sets the blendFunc, using separate parameters for the color and alpha components. They will only be applied when blending is enabled. 
	 * @return Whether the pending blend parameters were changed while the pending blending state is true. */
	...
	/** Sets the blend equations, using separate equations for the color and alpha components. They will only be applied 
